...
本環境は AWS 上に構築した Linux / Web / Database 基盤を対象とした運用設計ポートフォリオです。

構築だけでなく、以下の運用領域を含めて実装しています。

* Infrastructure as Code（Terraform）
* Linux運用（Rocky Linux）
* Webサーバ運用（Apache）
* Database運用（RDS PostgreSQL）
* パッチ運用（SSM Run Command）
* バックアップ／リストア
* 脆弱性管理
* CIS準拠コンフィグ評価
* 変更管理
...
実務における運用担当者～運用リーダーを想定した構成・成果物を作成しています。
...
```text
            ...
            │                                                     │
 ┌──────────▼──────────┐                          ┌──────────────▼─────────────┐
 │     Apache EC2      │                          │      WebLogic EC2          │
 │    Rocky Linux 9    │                          │      Rocky Linux 9         │
 │                      │                          │                            │
 │ ・Apache HTTPD       │                          │ ・Java 17                 │
 │ ・SSM Agent          │                          │ ・WebLogic設計演習         │
